...
const MON_YY = "Jul'26";
const JPY_USD_RATE = 160.5746; // Stay22用為替レート
...
// GA4
const ga4Lines = fs.readFileSync('src/data/actual_dl/202607.csv', 'utf8').split('\n');
const ga4Data = [];
...
if (fs.existsSync('src/data/actual_dl/202607_rakuten.csv')) {
  const rakutenLines = fs.readFileSync('src/data/actual_dl/202607_rakuten.csv', 'utf8').split('\n');
...
if (fs.existsSync('src/data/actual_dl/202607_stay22.csv')) {
  const stay22Lines = fs.readFileSync('src/data/actual_dl/202607_stay22.csv', 'utf8').split('\n');
...
// GSC
const googleLines = fs.readFileSync('src/data/actual_dl/202607_google.csv', 'utf8').split('\n');
const googleData = [];
...
// Bing
const bingLines = fs.readFileSync('src/data/actual_dl/202607_bing.csv', 'utf8').split('\n');
const bingData = [];
...
// 2. 前月比較データの読み込み
let prevReport = null;
if (fs.existsSync('src/reports/202606.json')) {
  prevReport = JSON.parse(fs.readFileSync('src/reports/202606.json', 'utf8'));
}
...
const julForecast = currentVintage.forecasts.find(f => f.m === MON_YY);

if (!julForecast) {
  throw new Error(`July forecasts not found in vintage ${vintagesData.currentVintageId}`);
}
...
STREAMS.forEach(s => {
  const forecastPv = julForecast.pv[s.key] || 0;
...
const totalForecastPv = STREAMS.reduce((sum, s) => sum + (julForecast.pv[s.key] || 0), 0);
...
const reportJson = {
  ...
  forecastComparison: {
    ...
    overallAccuracy,
    calibrationApplied: {
      newVintageId: `v${vintagesData.vintages.length}_202607`,
...
// レポートJSON書き出し
const reportFilePath = `src/reports/202607.json`;
...
// 新しいビンテージオブジェクト作成
const newVintageId = `v${vintagesData.vintages.length}_202607`;
...
vintagesData.currentVintageId = newVintageId;
...
console.log(`Updated forecast-vintages.json with new vintage: ${newVintageId}`);
...
if (insertIndex !== -1) {
  ...
  console.log(`Added July actual data to actual-pv.ts`);
} else {
